<div class="position-sticky top-0 z-3 bg-white border-bottom d-print-none" style="--bs-bg-opacity: .98;">
  <div class="container py-2 d-flex align-items-center gap-2 flex-wrap">
    ...
    <form class="d-flex flex-grow-1" role="search" action="{{ $indexUrl }}" method="get" data-index-url="{{ $indexUrl }}">
      <input type="search" class="form-control" placeholder="{{ $placeholder }}" aria-label="Cerca" id="globalSearchInput" value="{{ request('search') }}">
    </form>
  </div>
</div>
